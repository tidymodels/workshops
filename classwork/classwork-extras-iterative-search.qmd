...
Suppose that we are only tuning the learning rate in our neural network. We
...
show_best(init_res, metric = "brier_class", n = 3) |>
...
```{r}
show_best(nnet_bayes_res, metric = "brier_class") |>
  select(-.metric, -.estimator)
```
...
**Bonus points**: Before the optimization is done, press `<esc>` and see what
happens.
