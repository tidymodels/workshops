...
We calculate the mean of the outcome within each level of the predictor, and use
that as the new value.

Don't do just this! We are building up the method one thing at a time.
Unregularized target encoding is really prone to overfitting.
...
Calculate the global mean of the outcome and use it for cases that aren't seen
...
Apply target encoding to the data set, see how it affects different predictors,
not just the ones we listed here.
...
Apply date steps to the `arrival_date` variable and try to see if we capture
anything about `avg_price_per_room`.
...
Using `extrasteps::step_time_event()` and the
[almanac](https://davisvaughan.github.io/almanac/index.html) package, we can
...
Create a list of rules (last slide) and pass them to the `rules` argument of
[`extrasteps::step_time_event()`](https://emilhvitfeldt.github.io/extrasteps/reference/step_time_event.html)
