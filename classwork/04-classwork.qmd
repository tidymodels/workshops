...
"Resubstitution metric" - This should be the best possible performance that you could ever achieve, but it can be very misleading!
...
Use `augment()` and `metrics()` to compute regression metrics on `tree_fit`. Compute the metrics on both training and testing data. Notice the evidence of overfitting!

If you have time, try out some other yardstick metrics:
...
For an individual split, `analysis()` and `assessment()` can be used to extract out the corresponding data sets

```{r}
analysis(frog_folds$splits[[1]])
assessment(frog_folds$splits[[1]])
```
...
```{r}
# Don't forget the seed, for reproducibility
set.seed(123)
...
- Fit the random forest on each resample
- Keep predictions
- Compute assessment set metrics
...
```{r}
st_predictions <- predict(st, frog_test)

st_predictions <- frog_test %>%
  select(latency) %>%
  bind_cols(st_predictions)

st_predictions
```

```{r}
ggplot(st_predictions, aes(latency, .pred)) + 
  geom_abline(lty = 2, 
              col = "deeppink4", 
              size = 1.5) +
  geom_point(alpha = 0.5) +
