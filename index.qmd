...
-   A recent version of an IDE, either
    ...
    - Positron, available at <https://positron.posit.co/download.html>
-   For the _core slides_ (decks 1-9), the following R packages can be installed from the R console:
...
# fmt: skip
pkgs_core <- 
  c("dimRed", "embed", "extrasteps", "future", "igraph", "mirai",
...
-   One core package, [almanac](https://davisvaughan.github.io/almanac/), was archived from CRAN on 2026-08-22, so it has to be installed from GitHub:

```{r}
#| label: installs-almanac
#| eval: false
#| echo: true
...
# fmt: skip
pkgs_extras <- 
  c("bonsai", "brulee", "Cubist", "desirability2", "earth",
...
A few slides mention alternatives that need extra packages. You only need these if you want to work through those specific exercises: `rstanarm` (for `embed::step_lencode_bayes()`), `partykit` (for the conditional inference tree engine in deck 03), and `usethis` (only if you download the materials with `usethis::use_course()`).
...
-   08: [tidymodels and LLMs](slides/08-tidymodels-and-llms.html){target="_blank"}
-   09: [Wrapping up](slides/09-wrapping-up.html){target="_blank"}
...
-   [September 2026](archive/2026-09-posit-conf/index.html) at posit::conf()
