...
Make time to look at the `edge` column and think about how one would encode it
into numerics.
...
Figure out whether `step_dummy_extract()` or `step_dummy_multi_choice()` is most
appropriate on the `edge` variable in `leaf_data` and apply it.
...
The `predictor_prop` argument is used to determine how many zeroes in the
loadings.
