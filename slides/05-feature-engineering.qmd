...
* The **sensitivity** is the _true positive rate_ (accuracy on actual events).

* The **specificity** is the rate of correctly predicted non-events, or 1 - _false positive rate_.

# Two class data

These definitions assume that we know the threshold for class predictions (is 50% good?). 

What happens if we say that we need to be 80% sure to declare an event? (sensitivity `r emo::ji("down_arrow")`, specificity `r emo::ji("down_arrow")`)

What happens for a 20% threshold? (sensitivity `r emo::ji("up_arrow")`, specificity `r emo::ji("down_arrow")`) 
...
Use the area under the curve as a metric (1 = `r emo::ji("100")` 1/2 = `r emo::ji("thumb")`)
...
```{r roc-curve, echo = FALSE, out.width = '60%', fig.width = 5, fig.height = 5, fig.align='center', dev = 'svg', dev.args = list(bg = "transparent")}
autoplot(roc_curve_points)
```
