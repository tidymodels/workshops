...
before <- nhl_train %>% select(on_goal, player) %>% slice(1:7) %>% add_rowindex()
after <- left_join(before, estimates, by = "player") %>% 
  select(on_goal, player = estimate, .row)
```
...
nhl_glm_set_res <-
  workflow_set(
    list(dummy1 = nhl_indicators, angle2 = nhl_angle_rec, 
         dist3 = nhl_distance_rec, bgl4 = nhl_behind_rec),
