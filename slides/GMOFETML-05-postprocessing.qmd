...
Can we make our predictions better? Some examples: 
...
Let's first consider the easiest case. 

## Alternative thresholds

Instead of up-weighting the samples in the minority (via `class_weights`), we can try to fit the best model we can and then define what it means to be an "event." 
...
If we want to tune the threshold, how do we do that? 
...
There is a `fit()` function that requires data and the names of the prediction columns: 
...
## Manual use of a tailor  `r hexes(c("tailor", "workflows"))`
...
- Adjustment order matters, and errors may occur if there is a violation. 
...
- When there is estimation, the data considerations become more complex. 
...
## Grid results
 ...
 - To get a better threshold estimate, we could run another grid with values <2%.
...
nnet_thrsh_res |>
  ...
  relocate(threshold, sensitivity, specificity, brier_class)
```
...
The calibration issue in the previous plot shows that very likely non-events will have underestimated probabilities.
...
Let's say that we pick a threshold of 2%. Our explanation to the consumer of the results would be 
   
   > "As long as the model is at least 2% sure it is an event, we will call it an event". 
   
It may not be easy to convince someone that this is the best option. 
...
It would be a bad idea to fit a calibration model to the training set re-predictions. 
...
2. Use a static calibration set that lives outside our training/validation/testing splits. 

Currently, we have the first method implemented. 
...
Calibration models, in essence, try to predict the true class using the model predictions. 
...
Keep expectations low. In order for these methods to work: 
...
In many cases, it would be better to try a different model or tuning parameters. 
...
- Choose a specific activation type, based on previous results
- Add a calibrator to your tailor with a value of `tune()`. 
- Run another gird search
