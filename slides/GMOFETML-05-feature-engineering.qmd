---
title: "5 - Feature engineering: dummies and embeddings"
...
Every preprocessing step in a recipe that involves calculations uses the *training* set. For example:
...
Once a recipe has been estimated, there are various bits of information saved in it.
...
```{r}
#| label: fig-area-vs-equivalent-diameter
leaf_data |>
...
```{r}
#| label: fig-mean-red-val-vs-mean-blue-val
#| echo: false
leaf_data |>
...
```{r}
#| label: fig-physiological-length-vs-diameter
#| echo: false
leaf_data |>
...
NNMF, on the other hand, jointly optimizes all the components under the constraint that all the loadings are non-negative. While the data is also non-negative.
