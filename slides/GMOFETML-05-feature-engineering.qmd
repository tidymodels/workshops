...
Techniques that remove or alter features in order to have fewer more informative features.
...
Or duplicated or functionally equavilant features
...
Only exploration and trial and errors can determine whether you should use dimensionality reduction techniques. Knowing which methods does what helps you determine what to try.
...
This is not possible with t-SNE as it is an itterative methods that shifts observations in the lower dimensional space based on their distances to points in the higher dimensional space.
...
Luckily this can be solved using techniques from Linear Algebra more specifically, it can be solved using a eigen decomposition.
...
- Computational time is linear in number of columns
...
PCA aims to generate uncorrelated components that maximizes the variances. One component at a time.
...
the data has to be non-negative, e.i. 0 or higher.

This might feel like a pretty big restriction. And that is not wrong. But a lot of data sets ends up being naturally non-negative.

Or could at least be turned into non-negative ones with transformations.

It make for much easier interpretations as the loadings don't cancel each other out like they do for PCA.
