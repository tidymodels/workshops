...
leaf_rec <- recipe(species ~ ., data = leaf_data) |>  
\ \ step_dummy(all_nominal_predictors()) |>  
\ \ step_zv(all_predictors()) |>  
\ \ step_log(compactness, offset = 0.5) |>  
...
:::{style="font-family: 'Source Code Pro', monospace; font-size: 0.8em;"}
leaf_rec <- [recipe(species ~ ., data = leaf_data)]{style="color: #CA225E;"} |>  
\ \ step_dummy(all_nominal_predictors()) |>  
\ \ step_zv(all_predictors()) |>  
\ \ step_log(compactness, offset = 0.5) |>  
\ \ step_normalize(all_numeric_predictors())
...
\ \ [step_log]{style="color: #CA225E;"}(compactness, offset = 0.5) |>  
...
\ \ step_log([compactness]{style="color: #CA225E;"}, offset = 0.5) |> 
...
:::{style="font-family: 'Source Code Pro', monospace; font-size: 0.8em;"}
leaf_wflow <- workflow() |>  
\ \ [add_recipe(leaf_rec)]{style="color: #CA225E;"} |>  
...
- If you have an error and need to debug your recipe, the original recipe object (e.g. `leaf_rec`) can be estimated manually with a function called `prep()`. It is analogous to `fit()`. See [TMwR section 16.4](https://www.tmwr.org/dimensionality.html#recipe-functions).
...
```{r}
#| label: leaf_rec-prep-bake-printing
prep(leaf_rec) |>
  bake(new_data = leaf_data)
```
...
Is a way to transform a single numeric predictor into multiple numeric predictors, with the hope that the new numeric predictors are more linearily related to the outcome.

Mostly needed with linear models, but should rarely hurt to use.

## Splines explained

A B-spline is a piece-wise polynomial function.

We have 2 main parameters to worry about. number of knots and the polynomial degree.

The domain of the predictor is split into `k` regions, with knot between each, and a polynomial function is fit within each region, under the constraint that they touch each other at the knot.
...
- adds more columns
- will need to select number of columns
- can be messy outside range
