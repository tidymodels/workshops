...
#| echo: false
library(tidymodels)
load("2026_class_data.RData")

set.seed(123)
example_split <- initial_split(class_data)
...
rng_1 <- extendrange(example_train$pred_1)
rng_2 <- extendrange(example_train$pred_2)
seq_n <- 250
example_grid <- 
  crossing(
...
lr_grid <- 
  ...
  augment(new_data = example_grid) |> 
  mutate(Model = "Logistic Regression") |> 
  select(.pred_class_1, pred_1, pred_2, Model)
...
bound_base <- 
  ...
  geom_point(aes(col = class, pch = class), alpha = 2 / 3, cex = 2.5) + 
  coord_obs_pred() + 
...
<br> <!-- This is because the title in the previous slide spills onto a second line; adding this so that the image to compare to doesn't "bump down" on slide transition. -->
...
:::notes
The source of data leakage here is preprocessing on the whole source data set rather than on the whole training set. So, the risk is "the values from the test set affect the mean and standard deviations used in the preprocessing." That discussion shouldn't mention metrics, as they haven't been introduced yet.

While centering + scaling are easy to explain the general idea with, it's not necessarily the biggest danger in practice. The more elaborate the estimation in your preprocessing, the higher the risk of overfitting coming back to bite you. An example of a more elaborate preprocessing estimation is principal components.
:::
...
-   [Most importantly]{.underline}, a workflow captures the entire modeling process: `fit()` and `predict()` apply to the pre/post processing steps in addition to the actual model fit
...
`predict(tree_fit, new_data = example_test)`
...
`augment(tree_fit, new_data = example_test)`
