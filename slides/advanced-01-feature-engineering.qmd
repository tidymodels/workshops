...
- The recipe will use the _tring_ set to estimate the means and standard deviations of the data.
...
-   lump agents who rarely occur into an "other" group
...
The `agent` column is replaced with the log-odds of being on goal. 
...
:::notes
Partial pooling gives better estimates for players with fewer shots by shrinking the estimate to the overall on-goal rate (`TODO)
...
Not much worse but it can handle new agents (if they occur).
...
roc_curves_part_5 <- 
  hotel_lm_set_res %>% 
  collect_predictions() %>% 
  select(wflow_id, avg_price_per_room, .pred_yes) %>% 
  bind_rows(
    hotel_lm_res %>% 
      collect_predictions() %>% 
  ...
  group_by(wflow_id) %>% 
  roc_curve(avg_price_per_room, .pred_yes)
