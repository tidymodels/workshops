...
That do you think are in the `type` columns for the `lead_time` and `country` columns?
...
- converts them to integer hash value
- uses the integer to assign them to a specific indicator column. 
...
[Hash functions](https://www.metamorphosite.com/one-way-hash-encryption-sha1-data-software) are used in cryptology and are meant to _emulate_ randomness. 


## Feature Hashing Pros and Cons

- The procedure will automatically work on new values of the predictors.
- The is no real logic behind which factor levels are combined. 
- It is fast. 
...
tidy(hash_rec_fit, number = 1) %>% slice(1:4)
