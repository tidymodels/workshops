...
```{r}
#| echo: false
countdown(minutes = 3, id = "make-grid")
```
...
```{r}
#| echo: false
countdown(minutes = 3, id = "tune-glm")
```
...
-   high-degree spline for `distance` (more "wiggly", more complex)
...
```{r}
#| echo: false
countdown(minutes = 3, id = "select-by-pct-loss")
```
...
```{r}
select_best(glm_res, metric = "roc_auc")
select_by_pct_loss(glm_res, distance, metric = "roc_auc")
```
...
## Boosted trees `r hexes(c("recipes", "workflows", "parsnip", "tune"))`
...
```{r}
#| echo: false
countdown(minutes = 3, id = "xgb-wflow")
```
...
```{r}
#| echo: false
countdown(minutes = 3, id = "tune-xgboost")
```
...
show_best(xgb_coord_res, metric = "roc_auc", n = 1) %>% 
...
```{r}
#| echo: false
countdown(minutes = 20, id = "improve-xgb")
```
...
best_auc <- select_best(xgb_coord_res, metric = "roc_auc")
best_auc
...
## The final fit `r hexes(c("workflows", "tune"))`
...
```{r}
#| echo: false
countdown(minutes = 20, id = "finalize-xgb")
```
