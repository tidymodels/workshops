...
It has an additional `initial` argument for the initial set of performance estimates and parameter combinations for the GP model. This can be the results of another `tune_*()` function or an integer (in which case `tune_grid()` is used under to hood to make such an initial set of results).
...
What do we do now? 
  ...
  * Finalize the model by choosing the values for the tuning parameters. 
...
Compute your test set results and plot the observed and predicted daily room rates. 

```{r}
#| echo: false
countdown(minutes = 5, id = "test-res", left = "0")
```
