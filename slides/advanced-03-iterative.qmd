...
It has an additional `initial` argument that can take an integer or the previous results of another `tune_*()` function. 
...
What do we do now? 
  ...
  * Finalize the tuning parameters. 
...
# the predict() or augment() then compute metrics
```
