...
- Tomorrow we will walk thought an example data set that illustrates feature engineering and model tuning. 
...
- Our first model will have `r ncol(season_2015) - 1` simple features. 
...
plot_roc <- function(x, highlight = "dummy_logistic") {
  ...
  p <- 
    ...
    geom_step(data = emphasize, lwd = 1) +
    coord_fixed()
...
## Nonlinear terms for angle and distiance 
...
Once we find an acceptable model, the process is to 
