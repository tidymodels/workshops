...
```{r}
#| fig-align: 'center'
ggplot(taxi_train, aes(x = local, fill = tip)) +
...
```{r echo = FALSE}
#| fig-align: 'center'
quartiles <- quantile(taxi_train$distance, probs = c(1:3)/4)

ggplot(taxi_train, aes(distance)) +
  geom_histogram(bins = 20) +
  facet_grid(vars(tip)) +
...
:::notes
Based on our exploration, we realized that stratifying by distance might help get a consistent distribution. For instance, we'd include high and low distance in both the test and training
:::
...
```{r}
set.seed(123)
taxi_split <- initial_split(taxi, prop = 0.8, strata = distance)
taxi_split
```
