...
![](images/boosted-trees.gif){fig-align="center" width="100%"}
...
Look at the _engine documentation_ for xgboost models. 

```{r ex-xgboost}
#| echo: false
countdown::countdown(minutes = 5, id = "xgboost")
```
...
- Give their information to `tune_grid()`, etc.
...
## What is in `final_res`? `r hexes("tune")`

```{r collect-metrics-final-res}
...
```{r collect-predictions-final-res}
collect_predictions(final_res)
```


## What is in `final_res`? `r hexes(c("probably", "tune"))`
...
bst_grid <- augment(final_res |> extract_workflow(), new_data = cls_grid)
