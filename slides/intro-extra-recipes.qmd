...
:::{style="font-family: 'Source Code Pro', monospace; font-size: 0.8em;"}
rec_spec <- [recipe(tip ~ ., data = taxi_train)]{style="color: #CA225E;"} %>%  
\ \ step_unknown(all_nominal_predictors()) %>%  
\ \ step_dummy(all_nominal_predictors()) %>%  
\ \ step_zv(all_predictors()) %>%  
\ \ step_log(distance, offset = 0.5) %>%  
...
```{r}
#| label: rec_spec-prep-bake-printing
prep(rec_spec) %>%
...
Once a recipes as been estimated, there are various bits of information saved in it.
