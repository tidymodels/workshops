...
`update_role_requirements()` is required to make sure that this column is required when making new data points. 
...
The columns in the `stations` vector have a very high degree of correlation. 
...
## Plot the test set `r hexes(c("ggplot2", "stacks"))`
...
cubist_res %>% 
  ...
  geom_abline(lty = 2, col = "darkgreen") + 
  coord_obs_pred()
```
