...
# Start troch to bypass its effect on the RNGs
...
This will push the minority class probability estimates to be more accurate/[calibrated](https://aml4td.org/chapters/cls-metrics.html#sec-cls-calibration). _Overall_ the model will be less effective; this assumes the minority class is the class of interest. 
...
nnet_ex_spec <- 
  mlp(hidden_units = 20, penalty = 0.01, learn_rate = 0.005, epochs = 200) |> 
  set_engine("brulee", class_weights = 3, stop_iter = 5) |> 
...
Kind of? If sensitivity is important, then the model is moderately successful.  
...
nnet_spec <- 
  mlp(hidden_units = tune(), penalty = tune(), learn_rate = tune(), 
      epochs = 200, activation = tune()) |> 
  set_engine("brulee", class_weights = tune(), stop_iter = 5) |> 
  set_mode("classification")
...
par_text <- 
  cli::format_inline(
    "was reduced from {prettyunits::pretty_sec(seq)} to {prettyunits::pretty_sec(par)}, a speed-up of {round(seq / par, 1)}-fold")
```
...
That's is counter-producitve to finding the rare class "A" events.
...
::: notes
The curve hugging the bottom of the plot means that we are drastically _overestimating_ the probability of class "A" relative to how often it occurs "in the wild."
...
In the _postprocessing_ slides, we'll look at a alternative approach later that makes a different kinds of tradeoff. 
