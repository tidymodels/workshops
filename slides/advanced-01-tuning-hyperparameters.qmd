...
* The number of trees (`trees` in `[1, Inf]`)
* The number of samples needed to further split (`min_n` in `[1, nrow(x)]`).
* The rate that each tree adapts form previous iterations (`learn_rate` in `(0, 0.1)`).
* The number of iterations of boosting where _no improvement_ was shown (`early_stop`)
...
What if we used early stopping instead of set number of trees? 
