...
These data have a time component, and while not a typical time series data set, we might have used a time series resampling method. 

An example is shown in the extra slides “Case Study on Transportation”.

Take the agent data. Cross-validation may not group all of an agent’s data into the analysis or assessment sets. In this case, our analysis data might have future data that is after the agent’s data in the assessment set. 
