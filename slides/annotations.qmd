...
Early stopping is when we monitor the model's performance. If the model doesn't improve for `stop_iter` iterations, training stops. 
...
We _could use_ an engine argument called `validation` here. However, that's not an argument for any function in the lightgbm package. 
