...
⚠️ we don't want to use the test set until the end
...
```{r}
tunable_wflow <- workflow(latency ~ ., tunable_spec)
tunable_wflow
```
...
```{r, error = TRUE}
fit_resamples(tunable_wflow, frog_folds)
```
...
```{r}
set.seed(1)
tune_grid(tunable_wflow, frog_folds)
```
...
```{r}
set.seed(1)
tune_res <- tune_grid(tunable_wflow, frog_folds)
...
tree_wflow <- workflow(latency ~ ., tree_spec)
set.seed(1)
tree_res <- tune_grid(tree_wflow, frog_folds)
