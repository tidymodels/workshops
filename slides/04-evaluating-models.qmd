...
[AML4TD](https://aml4td.org/chapters/cls-metrics.html#sec-cls-hard-metrics)
:::
...
::: {.absolute bottom=0 right=0}
[AML4TD](https://aml4td.org/chapters/cls-metrics.html#sec-metric-information)
:::
...
- We can't really know if it under- or overfits until we have different data for model _fitting_ and model _evaluation_.
...
::: {.absolute bottom=0 right=0}
[AML4TD](https://aml4td.org/chapters/resampling.html#sec-cv)
:::
...
#| echo: false
countdown::countdown(minutes = 5, id = "try-refit-resamples")
```
...
## Evaluating model performance `r hexes(c("tune"))`

`collect_metrics()` shows us the overall resampling estimates of performance: 

<br> 

```{r}
#| label: fit-resamples-metrics

collect_metrics(cls_res)
```
...
library(probably)
cal_plot_windowed(cls_res, step_size = 0.03)
```
...
# Save the assessment set results
cls_preds <- collect_predictions(cls_res) |> slice(1:5)
...
[AML4TD](https://aml4td.org/chapters/cls-metrics.html#sec-cls-two-classes)
:::
...
::: {.absolute bottom=0 left=0}
[AML4TD](https://aml4td.org/chapters/cls-metrics.html#sec-roc)
:::
