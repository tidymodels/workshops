...
tree_spec <- decision_tree(cost_complexity = 0.00001, mode = "classification")
...
You can use `metric_set()` to combine multiple calculations into one

```{r}
my_metrics <- metric_set(accuracy, specificity, sensitivity)

augment(taxi_fit, new_data = taxi_train) %>%
  my_metrics(truth = tip, estimate = .pred_class)
```
...
*Use `augment()` and and a metric function to compute a classification metric like `precision()`.*
...
```{r}
set.seed(322)
bootstraps(taxi_train, times = 10)
```
