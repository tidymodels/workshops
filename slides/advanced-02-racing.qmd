...
How to we eliminate tuning parameter combinations? 

There are a few methods to do so. We'll use one based on analysis of variance (ANOVA). 

_However_... there is typically a large effect due to resamples in the data. 
...
Any candidates whose bound does not include zero are discarded. 
...
::: {.column width="50%"}
Only `r sum(map_int(lgbm_race_res$.metrics, ~ nrow(.x) / 2))` models were fit (out of 250). 
...
* Get the grid from the example results
* Pass it in as the grid
