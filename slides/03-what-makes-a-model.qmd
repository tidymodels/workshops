...
#| fig.width: 8
#| fig.height: 7
ggplot(taxi_test, aes(distance, tip)) +
...
logistic_preds <- 
  ...
  mutate(tip_num = if_else(tip == "yes", 0, 1))

ggplot(logistic_preds, aes(distance, tip_num)) +
  geom_point(color = test_color, size = 3) +
  geom_point(aes(distance, .pred_yes), color = data_color, size = 3) +
  theme_bw(base_size = 18)
```
:::
...
$p(\mbox{tip}) = \frac{}{1 + e^{-(\beta_0 + \beta_1\cdot x_{\text{distance}})}}$
...
#| fig.width: 8
#| fig.height: 7
distance_rng <- range(taxi_train$distance)
distance_grid <- tibble(distance = seq(distance_rng[1], distance_rng[2], length.out = 500))
...
tree_preds <- 
  tree_fit %>%
  augment(new_data = taxi_train)

tree_line <- 
  tree_fit %>%
  augment(new_data = distance_grid)
```
...
ggplot(data = tree_preds, aes(distance, tip)) +
  geom_segment(aes(x = distance, xend = distance, 
                   y = tip, yend = .pred), 
               colour = train_color, alpha = 0.8) +
  geom_line(aes(x = distance, y = .pred), size = 2, alpha = 0.8, color = data_color) +
...
ggplot(data = NULL, aes(distance, tip)) +
  geom_segment(data = linear_preds,
               aes(x = distance, xend = distance, 
                   y = tip, yend = .pred), 
               colour = train_color, alpha = 0.8) +
  geom_smooth(data = taxi_train, method = "lm", 
